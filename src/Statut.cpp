...
void Statut::drawBoss(SDL_Surface *gpScreen)
{
    ...
    if (jauge == true || nivjauge < 50 || nivjauge2 < 60)
    {
        ...
        if (jauge == true && valjauge > 0 && nivjauge > 0 && !gpJeu->getStop())
            nivjauge--;
        if ((jauge == false || (gpJeu->getZone() == 25 && valjauge == 0 && valjauge2 > 0)) && nivjauge < 50 && !gpJeu->getStop())
            nivjauge++;
        if (valjauge2 > 0 && nivjauge2 > 0 && !gpJeu->getStop())
            nivjauge2--;
        if (valjauge2 == 0 && nivjauge2 < 60 && !gpJeu->getStop())
            nivjauge2++;
        if (gpJeu->getZone() == 25 && dec < 60 && valjauge > 0 && valjauge2 > 0 && !gpJeu->getStop())
            dec++;
        if (gpJeu->getZone() == 25 && dec > 0 && (valjauge == 0 || valjauge2 == 0) && !gpJeu->getStop())
            dec--;
        ...
        if (maxjauge == 0)
            maxjauge = 1;
        if (valjauge > maxjauge)
            valjauge = maxjauge;
        src.w = (valjauge * 90) / maxjauge;
        src.h = 10;
        dst.x = 115 - dec;
        dst.y = 220 + nivjauge;
        src.x = 5 + 90 - src.w;
        ...
        if (gpJeu->getZone() == 25 && nivjauge2 < 50)
        {
            ...
            if (maxjauge2 == 0)
                maxjauge2 = 1;
            if (valjauge2 > maxjauge2)
                valjauge2 = maxjauge2;
            src.w = (valjauge2 * 90) / maxjauge2;
            src.h = 10;
            dst.x = 115 + dec;
            dst.y = 220 + nivjauge2;
            src.x = 5 + 90 - src.w;
...
void Statut::drawMagie(SDL_Surface *gpScreen)
{
    ...
    if (oni)
    {
        ...
        src.y = 0;
        SDL_BlitSurface(imageStatut, &src, gpScreen, &dst);
        src.h = (gpJoueur->getOnijauge() * 32) / gpJoueur->getOnimax();
        src.w = 8;
        dst.x = 10;
        dst.y = 18 + 32 - ((gpJoueur->getOnijauge() * 32) / gpJoueur->getOnimax());
        src.x = 202;
        src.y = 8;
        SDL_BlitSurface(imageStatut, &src, gpScreen, &dst);
    }
    ...
    if (gpJoueur->hasObjet(O_LANTERNE))
    {
        ...
        src.h = (gpJoueur->getMagie() / (gpJoueur->getMagieMax() / 32));
        src.w = 8;
        if (oni)
            dst.x = 24;
        else
    ...
    case 8:
        src.x = 16;
        src.y = 85;
        if (gpJoueur->hasObjet(O_MASQUE) == 1)
        {
            src.x = 0;
            src.y = 102;
        }
    ...
    default:
        src.x = 0;
        if (gpJoueur->hasBouteille(obj - 9) == 3)
            src.x = 16;
        if (gpJoueur->hasBouteille(obj - 9) == 4)
            src.x = 32;
        if (gpJoueur->hasBouteille(obj - 9) > 1)
            src.y = 51;
        if (gpJoueur->hasBouteille(obj - 9) == 1)
            src.y = 68;
        break;
...
void Statut::drawVie(SDL_Surface *gpScreen)
{
    ...
    // life
    gpJeu->affiche(gpScreen, "- ЖИЗНЬ -", 249, 3);
