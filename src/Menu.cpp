...
void Menu::drawInventaire(SDL_Surface *gpScreen)
{
    ...
    gpJeu->affiche(gpScreen, "X", 22 - dec, 20);
...
void Menu::drawStatut(SDL_Surface *gpScreen)
{
    ...
    int i = 36;
    if (!gpJoueur->hasObjet(O_LANTERNE) || !gpJoueur->getOnilink())
        i += 8;

    gpJeu->affiche(gpScreen, "СОСТОЯНИЕ :", 152, 20 - dec);
    ...
    if (vm < 10)
        oss << "0";
    oss << vm;
    gpJeu->affiche(gpScreen, "ЗДОРОВЬЕ", 152, i - dec);
    gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, i - dec);
    i += 16;
    ...
    if (gpJoueur->hasObjet(O_LANTERNE))
    {
        ...
        if (mm < 10)
            oss << "0";
        oss << mm;
        gpJeu->affiche(gpScreen, "МАГИЯ", 152, i - dec);
        gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, i - dec);
        i += 16;
    }
    ...
    if (gpJoueur->getOnilink())
    {
        ...
        if (oo < 10)
            oss << "0";
        oss << oo;
        gpJeu->affiche(gpScreen, "ОНИ-ЛИНК", 152, i - dec);
        gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, i - dec);
        i += 16;
    }
    ...
    gpJeu->affiche(gpScreen, "СИЛА", 152, i - dec);
    gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, i - dec);
    i += 16;
    oss.str("");
    oss << gpJoueur->getDefense();
    gpJeu->affiche(gpScreen, "ЗАЩИТА", 152, i - dec);
    gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, i - dec);
    i += 16;
    oss.str("");
    ...
    if (s < 10)
        oss << "0";
    oss << s;
    gpJeu->affiche(gpScreen, "ВРЕМЯ", 152, i - dec);
    gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, i - dec);
    ...
    {
        src.x = 16 * (gpJoueur->getBouclier() - 1);
        src.y = 16;
        dst.x = 176;
        if (gpJoueur->getOni())
        {
    ...
    {
        src.x = 48 + 16 * (gpJoueur->getTunique() - 1);
        src.y = 16;
        dst.x = 196;
        if (gpJoueur->getOni())
            src.x = 96;
...
void Menu::drawCristaux(SDL_Surface *gpScreen)
{
    ...
    gpJeu->affiche(gpScreen, "ГРААЛЬ :", 86, 180 + dec);
    ...
    for (int i = 0; i < 5; i++)
    {
        if (gpJeu->getJoueur()->hasCristal(i))
            src.x = 16;
        else
            src.x = 32;
        SDL_BlitSurface(imageInventaire, &src, gpScreen, &dst);
